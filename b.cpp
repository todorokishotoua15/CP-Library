...
//const int64_t mod = 29996224275833;
...
{  
    ...
    while (!q.empty()) {
        ...
        if (depth[node] != currdep) {
            currlev.clear();
            int idd = 0;
            prod = 0;
            for (auto x : levels[currdep]) {
                id[x] = idd++;
                currlev.push_back(ways[x]);
                prod += ways[x];
            }
            currdep = depth[node];
        }
